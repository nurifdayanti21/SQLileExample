...
class StudentDBHelper(context: Context) : SQLiteOpenHelper(
    ...
    companion object {
        ...
        private const val SQL_CREATE_ENTRIES = "CREATE TABLE " + DBContract.UserEntry.TABLE_NAME + " (" + DBContract.UserEntry.COLUMN_NIM + " TEXT PRIMARYKEY," +
    ...
    fun readStudents(): ArrayList<StudentModel> {
        ...
        val cursor: Cursor?
        try {
            cursor = db.rawQuery("select * from " + DBContract.UserEntry.TABLE_NAME + " order by " + DBContract.UserEntry.COLUMN_NIM, null)
        } catch (e: SQLiteException) {
            db.execSQL(SQL_CREATE_ENTRIES)
            return ArrayList()
        }
